---
title: "Extending herald"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Extending herald}
...
logic without forking the package: **operators** (the building blocks
of rules) and **dictionary providers** (controlled-terminology
membership checks). This article walks through both.
...
| Wire a different report format                 | Custom `report()` method |
...
A dictionary provider answers one question: *does this value exist
in this codelist or registry?* CDISC CT, FDA SRS, MedDRA, WHODrug,
LOINC, SNOMED, and any sponsor-private codelist all surface through
the same protocol so rule operators do not need to know the source
format.
...
- `contains(value, field, ignore_case)` -- a function returning a
  logical vector parallel to `value`. **Required.**
- `info()` -- a closure that returns the metadata above.
- `lookup(value, field)` -- optional. Returns matching rows for
  richer rule diagnostics.

Use `new_dict_provider()` to construct one. It validates the contract
and sets the S3 class.
...
`data.frame` to `custom_provider()` for the common case (it returns
a full `herald_dict_provider`), or hand-build with `new_dict_provider()`
...
`register_dictionary()` puts a provider in the session registry so
every later `validate()` call sees it. Or pass it inline via
`validate(dictionaries = list(...))` for a fully self-contained run.
...
1. The `dictionaries=` argument of `validate()` (explicit override).
2. The session registry (set by `register_dictionary()`).
3. Cache-discoverable providers (e.g. SRS via `download_srs()`).
4. The bundled CDISC CT (always available via `ct_provider()`).
5. If still missing, the rule is skipped and the miss is recorded
   in `result$skipped_refs` with a hint to register it.
...
| `size_rows`   | optional | Used by `print()` and `list_dictionaries()`. |
| `lookup`      | optional | Returns one row per match for diagnostics. |
| `info()`      | auto     | Built by `new_dict_provider()` from the above. |

## Part 2 -- Custom operators

An *operator* is a small function that returns a logical vector
saying which rows of a dataset fail a check. Rules in the YAML
catalog reference operators by name and supply arguments. Examples
of built-in operators include `iso8601`, `unique_key`, `subset_of`,
and `regex_match`.
...
The pattern below is what any new op file in `R/ops-*.R` looks like.
The actual registration uses an internal helper invoked at package
load; the function body is the part you write.

```{r op-body, eval = FALSE}
...
- Return `NA` on missing inputs; let the engine route those into
  the advisory / skipped channels.
- Argument names (`column`, etc.) are exactly what the rule YAML
  passes via the operator's `arg_schema`.

### Cross-dataset operators

Some rules need a second dataset (e.g. "every USUBJID in AE must
exist in DM"). Resolve the reference through the run context:

```{r cross-op, eval = FALSE}
...
If the reference dataset is absent, herald automatically records
the miss to `ctx$missing_refs$datasets`. The user sees it in
...
(e.g. `R/ops-string.R`, `R/ops-temporal.R`). Register at the bottom
of the file so the operator is discoverable at package load.
