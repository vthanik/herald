---
title: "Migrating from Pinnacle 21"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Migrating from Pinnacle 21}
...
published rule sources -- SDTM-IG, ADaM-IG, Define-XML 2.1, and
the CDISC Library API. The differences are operational, not
substantive: rule selection, reporting formats, dependency footprint,
and how the tool fits into a reproducible R pipeline.
...
| Provide define.xml       | Browse to file in the GUI           | `read_define_xml("define.xml")` -> pass to `validate()` |
| Plug in MedDRA / WHODrug | Configure dictionaries page         | `meddra_provider()`, `whodrug_provider()`, `register_dictionary()` |
...
**herald**: read the Define-XML, hand it to `validate()`. The same
Define-XML is then used to derive the spec for `apply_spec()`.

```{r define-side-by-side, eval = FALSE}
...
**herald**: register providers explicitly, in code, with version
locked.

```{r dict-side-by-side, eval = FALSE}
dictionaries <- list(
...
re-running the script next year with the same dictionary version
gives the same findings.

## Severity differences

P21 publishes severities in its proprietary scheme:
**Reject / Error / Warning / Notice**. CDISC publishes severities as
**High / Medium / Low**. The herald catalog uses CDISC severities
verbatim and exposes a `severity_map=` knob to rewrite labels at run
time without touching the catalog.
...
Some P21 features have no direct herald equivalent today; the
project tracks these explicitly.
...
1. Inventory the P21 rule set you currently run (Reject/Error tier
   first).
2. Map each rule code to a herald `rule_id`. The prefix tells you
   the standard.
...
5. Add a CI gate using `is_blocking()` (see the *Cookbook* article)
   so high-impact findings stop the build.
