...
#' Convert an Apache Parquet file to XPT
#'
#' Reads an Apache Parquet file and writes it as a SAS transport (.xpt) file.
...
#' @param version XPT version: \code{5} (FDA standard, default) or \code{8}.
...
#'
#' @return \code{xpt_path} invisibly.
#'
#' @examples
...
#'   on.exit(unlink(c(pq, xpt)))
#'   write_parquet(dm, pq, dataset = "DM", label = "Demographics")
#'   parquet_to_xpt(pq, xpt)
#' }
#'
#' @seealso [xpt_to_parquet()] for the reverse, [parquet_to_json()], [read_parquet()],
#'   [write_xpt()].
#' @family io
#' @export
parquet_to_xpt <- function(parquet_path, xpt_path, version = 5L,
  ...
  write_xpt(data, xpt_path, version = version, dataset = dataset, label = label)
  invisible(xpt_path)
}
