...
.get_builder <- function(name) {
  if (!exists(name, envir = .BUILDER_REGISTRY, inherits = FALSE)) return(NULL)
...
.build_define_datasets <- function(def) {
  if (!inherits(def, "herald_define")) return(list())
  result <- list()
...
.builder_study_metadata <- function(def) {
  ...
  if (is.na(odm_node)) odm_node <- xml2::xml_find_first(doc, "/ODM")
  ...
  if (is.na(study_node)) study_node <- xml2::xml_find_first(doc, ".//Study")
  ...
  if (is.na(gv_node)) gv_node <- xml2::xml_find_first(doc, ".//GlobalVariables")
...
.builder_dataset_metadata <- function(def) {
  ...
  if (length(igd_nodes) == 0L) return(NULL)
  ...
  rows <- lapply(igd_nodes, function(n) {
    ...
    # Description / label
    desc_node <- xml2::xml_find_first(n, "*[local-name()='Description']/*[local-name()='TranslatedText']")
...
.builder_variable_metadata <- function(def) {
  ...
  for (ign in igd_nodes) {
    ...
    if (length(refs) == 0L) refs <- xml2::xml_find_all(ign, "ItemRef")
    for (ref in refs) {
  ...
  if (length(item_nodes) == 0L) item_nodes <- xml2::xml_find_all(mdv, "ItemDef")
  if (length(item_nodes) == 0L) return(NULL)
  ...
  rows <- lapply(item_nodes, function(n) {
    ...
    codelist_oid <- if (is.na(clr_node)) "" else .attr_val(clr_node, "CodeListOID")
...
.builder_codelist_metadata <- function(def) {
  ...
  if (length(cl_nodes) == 0L) cl_nodes <- xml2::xml_find_all(mdv, "CodeList")
  ...
  for (cl in cl_nodes) {
    ...
    for (item in items) {
      ...
      dec_node <- xml2::xml_find_first(item, "*[local-name()='Decode']/*[local-name()='TranslatedText']")
...
.builder_standards_metadata <- function(def) {
  ...
  if (length(std_nodes) == 0L) return(NULL)
...
.builder_valuelevel_metadata <- function(def) {
  ...
  if (length(wc_nodes) == 0L) return(NULL)
