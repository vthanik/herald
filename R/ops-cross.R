...
.cdisc_value_equal <- function(a, b) {
  ...
  a_is_date <- a_is_date |
    (numeric_path & nchar(av_c) == 4L & !grepl("\\.", av_c))
  b_is_date <- b_is_date |
    (numeric_path & nchar(bv_c) == 4L & !grepl("\\.", bv_c))
...
op_is_not_unique_relationship <- function(data, ctx, name, value) {
  ...
  if (length(group_by) == 0L) {
    ...
    outer_key <- do.call(
      paste,
      c(
        lapply(group_by, function(g) {
          .na_sent(.rtrim_na(data[[g]]))
        }),
        list(sep = "\x1f")
      )
    )
  ...
  has_na_grp <- if (length(group_by) == 0L) {
    rep(FALSE, n)
  } else {
...
op_does_not_have_next_corresponding_record <- function(data, ctx, name, value) {
  ...
  ref_ds <- .ref_ds(ctx, ref_ds_name)
  if (is.null(ref_ds) || is.null(ref_ds[[by_key]])) {
    return(rep(NA, n))
  }
...
.parse_ref_arg <- function(name, value) {
  if (is.list(value)) {
    list(
      ref_ds = toupper(as.character(value$reference_dataset %||% "")),
...
op_ref_col_empty <- function(data, ctx, name, value) {
  ...
  populated_keys <- names(by_key)[vapply(
    by_key,
...
op_max_n_records_per_group_matching <- function(
  data,
  ctx,
  name,
  value,
  ...
  if (length(gk) == 1L) {
    ...
    gkey <- do.call(
      paste,
      c(lapply(gk, function(k) as.character(data[[k]])), list(sep = "\u0001"))
    )
  }
...
.register_op(
  ...
  op_has_next_corresponding_record,
  meta = list(
    kind = "cross",
...
op_differs_by_key <- function(
  ...
  .paste_cols <- function(df, cols) {
    if (length(cols) == 1L) {
      return(.as_char(df[[cols]]))
    }
    do.call(
      paste,
      c(
        lapply(cols, function(c) .na_sent(.as_char(df[[c]]))),
        list(sep = "\x1f")
      )
    )
  }
  ...
  lut <- stats::setNames(
    .as_char(ref_ds[[reference_column]]),
...
op_less_than_or_equal_by_key <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
op_greater_than_by_key <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
op_greater_than_or_equal_by_key <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
.register_op(
  ...
  op_greater_than_or_equal_by_key,
  meta = list(
    kind = "cross",
...
op_study_day_mismatch <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
op_any_index_missing_ref_var <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
op_value_not_var_in_ref_dataset <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
op_subject_has_matching_row <- function(
  data,
  ctx,
  name,
  reference_dataset,
...
op_next_row_not_equal <- function(data, ctx, name, value) {
  ...
  if (!is.list(value)) {
    herald_error(
      c(
        "Bad {.arg value} for `next_row_not_equal`.",
...
op_attr_mismatch <- function(data, ctx, name, attribute, reference_dataset) {
  n <- nrow(data)
  ref_ds <- .ref_ds(ctx, reference_dataset)
  if (is.null(ref_ds)) {
    return(rep(NA, n))
...
op_shared_attr_mismatch <- function(
  data,
  ctx,
  attribute,
  reference_dataset,
...
op_dataset_label_not <- function(data, ctx, expected) {
  ...
  .dataset_level_mask(
    toupper(trimws(as.character(lbl))) !=
...
op_treatment_var_absent_across_datasets <- function(
  data,
  ctx,
  current_vars,
  reference_vars,
  ...
  for (tmpl in ref_templates) {
    in_phs <- phs[vapply(
      phs,
      function(p) {
        grepl(p, tmpl, fixed = TRUE)
      },
      logical(1L)
    )]
    if (length(in_phs) == 0L) {
      if (toupper(tmpl) %in% ref_cols_up) {
        ref_present <- TRUE
        break
      }
      next
...
op_value_not_in_subject_indexed_set <- function(
  data,
  ctx,
  name,
  reference_dataset,
  ...
  if (is.null(data[[key]]) || is.null(ref_ds[[ref_key]])) {
    return(rep(NA, n))
  }
...
.register_op(
  "value_not_in_subject_indexed_set",
...
op_not_equal_subject_templated_ref <- function(
  data,
  ctx,
  name,
  reference_dataset,
  ...
  for (i in seq_len(n)) {
    ...
    if (is.na(col)) {
      next
    } # index_cols had NA on this row
    lut <- lookups[[col]]
    if (is.null(lut)) {
      next
    } # ref dataset lacks that column
    s <- row_keys[[i]]
    if (is.na(s)) {
      next
    }
...
.register_op(
  "not_equal_subject_templated_ref",
...
op_supp_row_count_exceeds <- function(
  data,
  ctx,
  ref_dataset,
  qnam_pattern,
...
op_shared_values_mismatch_by_key <- function(
  data,
  ctx,
  reference_dataset,
  key = "USUBJID",
...
op_no_baseline_record <- function(
  data,
  ctx,
  name,
  flag_var,
...
op_base_not_equal_abl_row <- function(
  data,
  ctx,
  b_var,
  a_var,
  ...
  if (
    is.null(data[[b_var]]) || is.null(data[[a_var]]) || is.null(data[[abl_col]])
  ) {
    return(rep(NA, n))
  }

  grp_cols <- as.character(unlist(group_by %||% character(0L)))
  if (length(setdiff(grp_cols, names(data))) > 0L) {
    return(rep(NA, n))
  }
...
op_not_present_on_multiple_rows_within <- function(
  data,
  ctx,
  name,
  within = "USUBJID",
  ...
  vals <- as.character(data[[col]])
  grp <- if (grp_col %in% names(data)) {
    as.character(data[[grp_col]])
  } else {
    rep("", n)
  }
...
op_target_is_not_sorted_by <- function(
  data,
  ctx,
  name,
  order_by = "USUBJID",
