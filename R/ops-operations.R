...
.register_operation("domain_label", .op_operation_domain_label, meta = list(
  kind = "cross", summary = "Dataset label attribute as a scalar string.",
  returns = "scalar", cost_hint = "O(1)"
))
...
.register_operation("distinct", .op_operation_distinct, meta = list(
  kind = "cross", summary = "Unique non-NA values of a column from the target dataset.",
  returns = "array", cost_hint = "O(n)"
))
...
.register_operation("record_count", .op_operation_record_count, meta = list(
  kind = "cross", summary = "Number of rows in the target dataset.",
  returns = "scalar", cost_hint = "O(1)"
))
...
.register_operation("study_domains", .op_operation_study_domains, meta = list(
  kind = "cross", summary = "Uppercase dataset names in the current validate() call.",
  returns = "array", cost_hint = "O(1)"
))
.register_operation("dataset_names", .op_operation_study_domains, meta = list(
  kind = "cross", summary = "Alias for study_domains: uppercase dataset names.",
  returns = "array", cost_hint = "O(1)"
))
...
.register_operation("max_date", .op_operation_max_date, meta = list(
  kind = "temporal", summary = "Lexicographic max of ISO-8601 date strings in a column.",
  returns = "scalar", cost_hint = "O(n)"
))
.register_operation("min_date", .op_operation_min_date, meta = list(
  kind = "temporal", summary = "Lexicographic min of ISO-8601 date strings in a column.",
  returns = "scalar", cost_hint = "O(n)"
))
...
.op_operation_study_day <- function(data, ctx, params) {
  ...
  .sdtm_study_day <- function(dt_str, ref_str) {
    if (is.na(dt_str) || is.na(ref_str) || !nzchar(dt_str) || !nzchar(ref_str)) {
      return(NA_integer_)
    }
  ...
  }, as.character(data[[idx[[1L]]]]), subj, USE.NAMES = FALSE)
}

.register_operation("dy", .op_operation_study_day, meta = list(
  kind = "temporal",
  summary = "Per-row CDISC study day (--DY formula) relative to DM.RFSTDTC.",
  returns = "vector", cost_hint = "O(n)"
))
...
.op_operation_dataset_filtered_variables <- function(data, ctx, params) names(data)
...
.register_operation("get_codelist_attributes", .op_operation_get_codelist_attributes,
  meta = list(kind = "cross",
              summary = "Codelist submission values from the session CT provider.",
              returns = "array", cost_hint = "O(1)"))
.register_operation("extract_metadata", .op_operation_extract_metadata,
  meta = list(kind = "cross",
...
.register_operation("domain_is_custom", .op_operation_domain_is_custom, meta = list(
  kind = "cross",
