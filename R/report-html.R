...
write_report_html <- function(x, path, title = NULL, ...) {
  ...
  skipped <- x$skipped_refs %||% list(datasets = list(), dictionaries = list())
...
.html_header_meta <- function(
  ...
  paste0(
    ...
    'Rules exercised ',
    .fmt_int(rules_applied %||% 0L),
    ' of ',
    .fmt_int(rules_total %||% 0L),
    ' &middot; run time ',
    htmlesc(format(duration_secs)),
    's.',
    '</p>'
  )
}
...
.html_skipped_refs <- function(skipped) {
  ...
  if (length(ds_entries) == 0L && length(dict_entries) == 0L) {
    return("")
  }

  entry_row <- function(nm, e, kind_label) {
    rule_list <- paste(
      vapply(e$rule_ids %||% character(), htmlesc, character(1)),
      collapse = ", "
    )
    paste0(
      '<li class="sev-row">',
      ...
      ' rule',
      if (length(e$rule_ids %||% character()) == 1L) '' else 's',
      '</span>',
  ...
  rows <- character()
  for (nm in names(ds_entries)) {
    rows <- c(rows, entry_row(nm, ds_entries[[nm]], "Dataset"))
  }
  for (nm in names(dict_entries)) {
    rows <- c(rows, entry_row(nm, dict_entries[[nm]], "Dictionary"))
  }
...
.html_tab_issues <- function(counts, n_fired, n_adv) {
  ...
  paste0(
    ...
    '<ul class="sev-list">',
    paste(sev_rows, collapse = ""),
    '</ul>',
    '</section>',
    '<section>',
    '<h2 class="section-head">Most-cited rules</h2>',
    '<ol class="top-rules">',
    paste(top_rows, collapse = ""),
    '</ol>',
    '</section>',
    '</div>'
  )
...
.html_tab_details <- function(findings) {
  ...
  for (i in seq_len(nrow(rows))) {
    out[i] <- paste0(
      ...
      '">',
      '<td class="mono">',
      htmlesc(.na_blank(rows$rule_id[i])),
      '</td>',
      '<td class="sev-cell">',
      htmlesc(.na_blank(rows$severity[i])),
      '</td>',
      '<td>',
      htmlesc(.na_blank(rows$status[i])),
      '</td>',
      '<td class="mono">',
      htmlesc(.na_blank(rows$dataset[i])),
      '</td>',
      '<td class="mono">',
      htmlesc(.na_blank(rows$variable[i])),
      '</td>',
      '<td class="num">',
      .fmt_row(rows$row[i]),
      '</td>',
      '<td class="mono">',
      htmlesc(.na_blank(rows$value[i])),
      '</td>',
      '<td class="wide">',
      htmlesc(.na_blank(rows$message[i])),
      '</td>',
      '</tr>'
    )
  }
...
.html_tab_datasets <- function(ds_tbl) {
  ...
  for (i in seq_len(nrow(ds_tbl))) {
    out[i] <- paste0(
      '<tr>',
      '<td class="mono ink">',
      htmlesc(.na_blank(ds_tbl$name[i])),
      '</td>',
      '<td>',
      htmlesc(.na_blank(ds_tbl$class[i])),
      '</td>',
      '<td class="num">',
      .fmt_int(ds_tbl$rows[i]),
      '</td>',
      '<td class="num">',
      .fmt_int(ds_tbl$cols[i]),
      '</td>',
      '<td class="wide">',
      htmlesc(.na_blank(ds_tbl$label[i])),
      '</td>',
      '<td class="num">',
      .fmt_int(ds_tbl$fired_n[i]),
      '</td>',
      '<td class="num">',
      .fmt_int(ds_tbl$advisory_n[i]),
      '</td>',
      '</tr>'
    )
  }
...
.html_tab_rules <- function(rules_df) {
  if (nrow(rules_df) == 0L) {
    return(
      '<tr><td colspan="8" class="empty">No rules in the applied catalog.</td></tr>'
    )
  }
  ...
  for (i in seq_len(nrow(rules_df))) {
    ...
    out[i] <- paste0(
      ...
      '">',
      '<td class="mono ink">',
      htmlesc(.na_blank(rules_df$id[i])),
      '</td>',
      '<td class="sev-cell">',
      htmlesc(.na_blank(rules_df$severity[i])),
      '</td>',
      '<td>',
      htmlesc(.na_blank(rules_df$authority[i])),
      '</td>',
      '<td>',
      htmlesc(.na_blank(rules_df$standard[i])),
      '</td>',
      '<td class="num">',
      .fmt_int(rules_df$fired_n[i]),
      '</td>',
      '<td class="num">',
      .fmt_int(rules_df$advisory_n[i]),
      '</td>',
      '<td>',
      src_cell,
      '</td>',
      '<td class="wide">',
      htmlesc(.na_blank(rules_df$message[i])),
      '</td>',
      '</tr>'
    )
  }
