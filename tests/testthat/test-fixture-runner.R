...
.read_fixture <- function(path) {
  ...
  if (length(missing) > 0L) {
    stop(sprintf(
      "Fixture %s is missing field(s): %s",
      path,
...
.fixture_dictionaries <- function() {
  ...
  make <- function(name) {
    tbl <- as.data.frame(
      stats::setNames(rep(list("VALID"), length(fields)), fields),
      stringsAsFactors = FALSE
    )
...
.assert_fixture <- function(fx) {
  ...
  if (isTRUE(res$rules_applied == 0L)) {
    testthat::skip(sprintf(
      "rule %s not in catalog (superseded or not yet authored)",
      fx$rule_id
    ))
  }
  ...
  if (fires) {
    testthat::expect_gt(
      ...
      0L,
      label = sprintf(
        "fixture %s positive: rule %s should fire",
        basename(dirname(fx$`_path`)),
        fx$rule_id
      )
    )
    ...
    if (length(exp_rows) > 0L) {
      testthat::expect_true(
        ...
        label = sprintf(
          "fixture %s positive: expected row(s) %s in fired %s",
          fx$rule_id,
          toString(exp_rows),
    ...
    testthat::expect_equal(
      ...
      0L,
      label = sprintf(
        "fixture %s negative: rule %s should NOT fire",
        fx$rule_id,
        fx$rule_id
      )
    )
